...
const initialState = {
  localBasket: typeof window !== 'undefined' && localStorage.getItem('localBasket')
    ? JSON.parse(localStorage.getItem('localBasket')) || []
    : [],
  localTotalBasePrice: typeof window !== 'undefined' && localStorage.getItem('localTotalBasePrice')
  ...
  reducers: {
    addToBasket: (state, action) => {
      const item = action.payload;
            const existingItemIndex = state.localBasket.findIndex((i) => i.id === item.id);
      ...
      state.localBasket.forEach((product) => {
        newTotalPrice += product.price
          ...
          : Math.min(product.price, product.discounted_price) * product.quantity;
      });

      localStorage.setItem('localBasket', JSON.stringify(state.localBasket));
      localStorage.setItem('localTotalBasePrice', JSON.stringify(newTotalPrice));

      state.localTotalBasePrice = newTotalPrice;
    },
    removeFromBasket: (state, action) => {
      const item = action.payload;
      state.localBasket = state.localBasket.filter((a) => a.rd_id !== item.rd_id);
...
export const { addToBasket, removeFromBasket, clearBasket } = pointOfSellSlice.actions;
